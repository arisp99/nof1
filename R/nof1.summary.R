...
#changed "#ffb380" to "#ff8080" and added "#80ff8c". Issue was that there were not enough color options. Added na.rm=TRUE
#for geom_hline. number of categories is only used for ordinal data. so maybe time_series_plot only works for ordinal.
time_series_plot <- function(nof1, time = NULL, timestamp = NULL, timestamp.format = "%m/%d/%Y %H:%M", Outcome.name = ""){
  ...
  ggplot(data, aes(x=date, Y, fill = Treatment)) + geom_bar(stat = "identity", na.rm = TRUE)  + facet_grid(. ~ Treatment) + theme_bw() +
...
#changed the bin size (added bins=10) and added in na.rm = TURE. Seperated binomial and ordinal data.
frequency_plot <- function(nof1, xlab = NULL, title = NULL){
  ...
  if(nof1$response %in% c("binomial")){
    ...
    ggplot(data= data, aes(x= Y, y= x, fill=Treat)) +  geom_bar(stat="identity", position="dodge", width = 0.8, na.rm = TRUE) + labs(title = title, x = xlab, y = "Frequency", fill = "Outcomes") + xlim(-0.5, 1.5)+ theme_bw()
  } else if(nof1$response %in% c("ordinal")){
    data <- aggregate(nof1$Y, list(Y = nof1$Y, Treat = nof1$Treat), length)
    ggplot(data= data, aes(x= Y, y= x, fill=Treat)) +  geom_bar(stat="identity", position="dodge", na.rm = TRUE) + labs(title = title, x = xlab, y = "Frequency", fill = "Outcomes") + xlim(0.5, nof1$ncat +0.5)+ theme_bw()
  } else if(nof1$response %in% c("normal", "poisson")){
    data <- data.frame(Y = nof1$Y, Treat = nof1$Treat)
    ggplot(data, aes(x = Y, fill = Treat, color = Treat)) + geom_histogram(position = "dodge", na.rm = TRUE, bins = 10, alpha = 0.7) + labs(title = title, x = xlab) + theme_bw()
  }
}
...
stacked_percent_barplot <- function(nof1, title = NULL){
  ...
  if(nof1$response %in% c("binomial")){
    ...
    ggplot(data, aes(fill= factor(Y, levels = 0:1), y= x, x= Treat)) + geom_bar( stat="identity", position="fill", na.rm = TRUE) +
...
raw_table <- function(nof1){
  ...
  if(nof1$response %in% c("binomial", "ordinal")){
    ...
    cbind(raw_table, sd = aggregate(nof1$Y, list(Treat = nof1$Treat), sd, na.rm = TRUE)[,-1], aggregate(nof1$Y, list(Treat = nof1$Treat), quantile, na.rm = TRUE, c(0.025, 0.5, 0.975))[,-1])
  }
}
